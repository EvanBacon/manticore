...
const Constants = {
  linkingUrl: "exp://192.168.1.4:19000/",
 ...
 * **Examples**
 *
 * - Bare, Standalone: yourscheme://path
...
export function makeUrl(path = "", queryParams = {}) {
  ...
  console.log("TRY: ", { path, hostUri, isExpoHosted: isExpoHosted() });
  if (path) {
  ...
  hostUri = ensureLeadingSlash(hostUri, false);
  if (hostUri && path) {
    path = ensureTrailingSlash(path, true);
  }
...
export function parse(url) {
  ...
  const hostUri = getHostUri() || "";
  const hostUriStripped = removePort(
    removeTrailingSlashAndQueryString(hostUri)
  );
